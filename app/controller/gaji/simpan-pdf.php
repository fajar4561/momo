...
// $tahun_get = $_GET['tahun'];


// tahap pertama chek dahulu status transaksi gaji yang emailnya == 0
$koneksi_email = $koneksi->query("SELECT * FROM transaksi_gaji WHERE email = 0 ");
$ada_koneksi_email = $koneksi_email->num_rows;

if ($ada_koneksi_email == 1) {
    $data_email = $koneksi_email->fetch_assoc();
    $kode_transaksi_gaji = $data_email['kode_transaksi'];
    
    $folder = "../../../public/file/gaji/".$kode_transaksi_gaji."/";
    if (!file_exists($folder)) {
        if (mkdir($folder, 0777, true)) { // Buat folder dengan izin 0777
    ...
    // ambil data pegawai yang ada emailnya.
    $koneksi_pegawai = $koneksi->query("SELECT * FROM pegawai WHERE email != '' AND email != 'maulanafajar751@gmail.com'");

    // generated file ke pdf dan simpan ke server
    require('../../../public/plugins/fpdf/fpdf.php');
    function format_currency($value) {
        return $value != 0 ? 'Rp.' . number_format($value) : '';
    }

    while ($data_pegawai =mysqli_fetch_assoc($koneksi_pegawai)) {
        $nopeg = $data_pegawai['nopeg'];
        $kode = $kode_transaksi_gaji;
        ...
        $data = $koneksi->query("SELECT * FROM pegawai WHERE nopeg='$nopeg'")->fetch_assoc();
            ...
            $pdf->SetFont('Arial', '', 10);
            $pdf->Ln(17);
            $pdf->Cell(45,5,'SEMARANG,'.date("d-m-Y", strtotime($pecah['tgl_gaji'])),0,0);
            $pdf->Ln();

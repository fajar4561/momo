    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header">
                <div class="row">
                    <div class="col-sm-10">
                        <h4 class="card-title">Laporan Penyusutan Inventaris</h4>
                        <p class="text-muted mb-0">Halaman ini digunakan untuk mencetak laporan Penyusutan Barang yang dapat di cetak dengan beberapa parameter.
                        </p>
                    </div>
                </div>
            </div>
                                    ...
                                    <div class="col-12 mb-2">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <select class="form-select" id="inlineFormSelectPref" name="jenis" required onChange="tampil(this.value)">
                                                ...
                                                <optgroup label="Waktu Pembelian Barang">
                                                    <option value="1">Tahunan</option>
                                                    <option value="2">Bulanan</option>
                                                    <option value="3">Rentang Tanggal</option>
                                                </optgroup>
                                                <!-- <optgroup label="Lokasi Barang">
                                                    <option value="4">Unit</option>
                                                </optgroup>
                                                <optgroup label="Jenis Barang">
                                                    <option value="5">Katagori Barang</option>
                                                </optgroup> -->
                                            </select>
                                        </div>
                                    </div>
...
<?php 
// Hasilkan opsi dropdown dari database untuk opsi `unit`
$options_unit = '';
$sq = $koneksi->query("SELECT * FROM unit_inv ORDER BY unit ASC");
while ($data = mysqli_fetch_assoc($sq)) {
    $options_unit .= "<option value=\"{$data['unit']}\">{$data['unit']}</option>";
}

$options_barang = '';
$ambil_jenis= $koneksi->query("SELECT * FROM master_barang");
while ($data_barang = mysqli_fetch_assoc($ambil_jenis)) {
    $options_barang .=  "<option value=\"{$data_barang['id']}\">{$data_barang['keterangan']}</option>";
}
?>
...
function tampil(jenis) {
    ...
    switch (jenis) {
        ...
        case "5":
            tampil = `
                <div class="col-12 mb-2">
                    <select id="unit-select" class="form-control form-select" name="katagori" required>
...
<script>
    $(document).ready(function() {
        $('#laporanForm').on('submit', function(event) {
            ...
            $.ajax({
                url: 'app/print/inv/laporan-penyusutan.php',
                type: 'GET',
